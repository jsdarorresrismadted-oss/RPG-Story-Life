...
export default function UsersPage() {
  const { items: users, loading, reload } = useCrudList("users", () => adminApi.users.list());
  const { data: classesData } = useQuery({
    queryKey: ["classes"],
    queryFn: async () => (await adminApi.classes.list()).data,
  });
  ...
  const columns: DataTableColumn[] = [
    ...
    },
    {
      ...
      label: "Role",
      render: (v: string) => (
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${v === "admin" || v === "owner" ? "bg-accent-500/20 text-accent-400" : "bg-gray-600/20 text-gray-400"}`}>
          {v}
        </span>
      ),
    },
    { key: "level", label: "Level", render: (v: any) => v },
    ...
    { key: "pvpCoins", label: "PVP", render: (v: any) => <span className="font-mono">{v}</span> },
    { key: "gc", label: "GC", render: (v: any) => <span className="font-mono">{v}</span> },
    { key: "characters", label: "Characters", render: (_v: any, u: AdminUser) => u._count?.characters ?? 0 },
    {
      key: "status",
      label: "Status",
      render: (_v: any, u: AdminUser) => (
        <span className="flex items-center gap-1.5 text-xs">
          ...
          {u.isBanned ? <span className="text-red-400">Banned</span> : u.isOnline ? <span className="text-green-400">Online</span> : <span className="text-gray-500">Offline</span>}
        </span>
      ),
    },
    { key: "createdAt", label: "Joined", render: (v: string) => <span className="text-xs text-gray-500">{new Date(v).toLocaleDateString()}</span> },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
        <SearchInput value={search} onChange={(v) => { setSearch(v); setPage(0); }} placeholder="Buscar usuário..." />
      </div>
      ...
      <DataTable
        ...
        rowActions={(u) => (
          <>
            <button onClick={() => openDetail(u)} title="View inventory, classes and details" className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-white mr-3">
              <Eye size={14} /> Details
            </button>
            <button onClick={() => setEditing(u)} title="Edit user" className="inline-flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 mr-3">
              <Pencil size={14} /> Edit
            </button>
            <button onClick={() => toggleAdmin(u)} className="inline-flex items-center gap-1 text-xs text-accent-400 hover:text-accent-300 mr-3">
              {u.role === "admin" ? <ShieldOff size={14} /> : <ShieldCheck size={14} />}
              {u.role === "admin" ? "Revoke admin" : "Make admin"}
            </button>
            <button onClick={() => toggleBan(u)} className={`text-xs mr-3 ${u.isBanned ? "text-green-400 hover:text-green-300" : "text-red-400 hover:text-red-300"}`}>
              {u.isBanned ? "Unban" : "Ban"}
            </button>
            <button onClick={() => askDelete(u)} title="Delete user" className="inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300">
